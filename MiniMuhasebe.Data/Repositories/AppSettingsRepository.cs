    ...
    public class AppSettingsRepository : BaseRepository<AppSetting>
    {
        ...
        public override AppSetting GetById(int id)
        {
            ...
            using (var connection = new SQLiteConnection(_connectionString))
            {
                ...
                {
                    if (reader.Read()) return MapSetting(reader);
                }
            }
            return null;
        }
        ...
        public bool SetValue(string key, string value)
        {
            ...
            if (existing != null)
            {
                string query = "UPDATE AppSettings SET SettingValue = @Value, UpdatedAt = @UpdatedAt WHERE SettingKey = @Key";
                return ExecuteNonQuery(query,
                    new SQLiteParameter("@Value", value),
                    new SQLiteParameter("@UpdatedAt", DateTime.Now),
                    new SQLiteParameter("@Key", key)) > 0;
            }
            else
            {
                string query = "INSERT INTO AppSettings (SettingKey, SettingValue, SettingType, UpdatedAt) VALUES (@Key, @Value, 'String', @UpdatedAt)";
                return ExecuteNonQuery(query,
                    new SQLiteParameter("@Key", key),
                    new SQLiteParameter("@Value", value),
                    new SQLiteParameter("@UpdatedAt", DateTime.Now)) > 0;
            }
        }
        ...
        public override List<AppSetting> GetAll()
        {
            ...
            using (var connection = new SQLiteConnection(_connectionString))
            {
                ...
                {
                    while (reader.Read()) settings.Add(MapSetting(reader));
                }
            }
            return settings;
        }
        ...
        {
            string query = @"INSERT OR IGNORE INTO AppSettings (SettingKey, SettingValue, SettingType, Description, UpdatedAt)
