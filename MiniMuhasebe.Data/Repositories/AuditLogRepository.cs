...
namespace MiniMuhasebe.Data.Repositories
{
    public class AuditLogRepository : BaseRepository<AuditLog>
    {
        ...
        {
            var logs = new List<AuditLog>();
            string query = "SELECT * FROM AuditLogs WHERE Action = @Action ORDER BY CreatedAt DESC";
            using (var connection = new SQLiteConnection(_connectionString))
            {
                connection.Open();
                using (var reader = ExecuteReader(query, connection, new SQLiteParameter("@Action", action)))
                {
                    while (reader.Read())
                        logs.Add(MapAuditLog(reader));
                }
        ...
        {
            string query = "DELETE FROM AuditLogs WHERE AuditLogId = @AuditLogId";
            return ExecuteNonQuery(query, new SQLiteParameter("@AuditLogId", id)) > 0;
        }

        /// <summary>
        /// Belirli tarihten eski logları temizle
        /// </summary>
        public int DeleteOlderThan(DateTime cutoffDate)
        {
            string query = "DELETE FROM AuditLogs WHERE CreatedAt < @CutoffDate";
            return ExecuteNonQuery(query, new SQLiteParameter("@CutoffDate", cutoffDate));
        }
