    ...
    public class CurrentAccountTransactionRepository : BaseRepository<CurrentAccountTransaction>
    {
        ...
        public override CurrentAccountTransaction GetById(int id)
        {
            ...
            using (var connection = new SQLiteConnection(_connectionString))
            {
                ...
                {
                    if (reader.Read()) return MapTransaction(reader);
                }
            }
            return null;
        }
        ...
        {
            string query = @"SELECT COALESCE(SUM(CASE WHEN TransactionType = 'Debit' THEN Amount ELSE -Amount END), 0)
                           FROM CurrentAccountTransactions WHERE CurrentAccountId = @CurrentAccountId";
            var result = ExecuteScalar(query, new SQLiteParameter("@CurrentAccountId", currentAccountId));
        ...
        public override int Add(CurrentAccountTransaction entity)
        {
            ...
            var parameters = new SQLiteParameter[]
            {
                ...
                new SQLiteParameter("@TransactionDate", entity.TransactionDate.ToString("yyyy-MM-dd")),
        ...
        {
            string query = @"UPDATE CurrentAccountTransactions SET TransactionDate = @TransactionDate, Amount = @Amount,
                           TransactionType = @TransactionType, Description = @Description, RelatedDocumentNumber = @RelatedDocumentNumber,
                           Notes = @Notes WHERE TransactionId = @TransactionId";
            var parameters = new SQLiteParameter[]
            {
                new SQLiteParameter("@TransactionDate", entity.TransactionDate.ToString("yyyy-MM-dd")),
                new SQLiteParameter("@Amount", entity.Amount),
