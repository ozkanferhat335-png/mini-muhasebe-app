    ...
    public class FiscalPeriodRepository : BaseRepository<FiscalPeriod>
    {
        ...
        public override FiscalPeriod GetById(int id)
        {
            ...
            using (var connection = new SQLiteConnection(_connectionString))
            {
                ...
                {
                    if (reader.Read()) return MapPeriod(reader);
                }
            }
            return null;
        }
        ...
        public List<FiscalPeriod> GetByCompanyId(int companyId)
        {
            ...
            using (var connection = new SQLiteConnection(_connectionString))
            {
                ...
                {
                    while (reader.Read()) periods.Add(MapPeriod(reader));
                }
            }
            return periods;
        }

        public FiscalPeriod GetActiveByCompanyId(int companyId)
        {
            string query = "SELECT * FROM FiscalPeriods WHERE CompanyId = @CompanyId AND IsClosed = 0 ORDER BY StartDate DESC LIMIT 1";
            using (var connection = new SQLiteConnection(_connectionString))
            {
                connection.Open();
                using (var reader = ExecuteReader(query, connection, new SQLiteParameter("@CompanyId", companyId)))
        ...
        private FiscalPeriod MapPeriod(SQLiteDataReader reader)
        {
