...
namespace MiniMuhasebe.Data.Repositories
{
    public class TransactionMatchRepository : BaseRepository<TransactionMatch>
    {
        ...
        public TransactionMatch GetByIncomeExpenseTransactionId(int incomeExpenseTransactionId)
        {
            string query = "SELECT * FROM TransactionMatches WHERE IncomeExpenseTransactionId = @IncomeExpenseTransactionId LIMIT 1";
            using (var connection = new SQLiteConnection(_connectionString))
            {
                connection.Open();
        ...
        public bool ExistsMatch(int bankTransactionId, int incomeExpenseTransactionId)
        {
        ...
        {
            string query = @"UPDATE TransactionMatches SET MatchScore = @MatchScore, MatchType = @MatchType 
            ...
            var parameters = new SQLiteParameter[]
            {
                new SQLiteParameter("@MatchScore", entity.MatchScore ?? (object)DBNull.Value),
                new SQLiteParameter("@MatchType", entity.MatchType ?? "Manual"),
                new SQLiteParameter("@MatchId", entity.MatchId)
            };
